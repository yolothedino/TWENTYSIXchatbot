...
int chatbot_do_load(int inc, char *inv[], char *response, int n) {
	...
	char catbuf[MAX_INPUT] = "";
	for (int i = 1; i < inc; i++)	//concatenates every word after the first word (load) to be a single char array
	{
		if (strcmp(catbuf, "") == 0)
			strcpy(catbuf, inv[i]);
		else
	...
	fp = fopen(catbuf, "r");
	if (fp == NULL) {
		snprintf(response, n, "Filename \"%s\" not found. Be sure to enter .ini after the file's name. e.g \"MyFileName.ini\". Further ensure that the file is located within the same folder as the program.", catbuf);
	}
	else {
		int count = knowledge_read(fp, botName);		//knowledge_read returns the amount of knowledge lines read

		if (count == 0)		//no knowledge was acquired
			snprintf(response, n, "Read no knowledge. Please make sure %s is a file with a compatible structure and format.", catbuf);
		else
		...
		if (compare_token(inv[0], "what") == 0 || compare_token(inv[0], "where") == 0 || compare_token(inv[0], "who") == 0)
		{
			...
			else if (compare_token(inv[1], "is") != 0 || compare_token(inv[1], "are") != 0)
			{
				...
				else if (knowledge_get(inv[0], catbuf, response, n) == KB_NOTFOUND)
				{
					...
					if (tempbuf[0] == '\n')
					{
						snprintf(response, n, "No answer detected. Please ask again.");
						return 0;
					}
					else
					{
...
int chatbot_do_save(int inc, char* inv[], char* response, int n) {
	...
	char* isFound;
	isFound = strstr(catbuf, ".ini");

	if (fp == NULL)
		snprintf(response, n, "Please enter a file name.");		//likely won't happen, but still
	else if (isFound == NULL)	//.ini not found
		snprintf(response, n, "Lack of .ini file format detected. Please enter .ini as the file extension e.g \"MyFileName\".ini>.");
	else
	{
		fwrite("[NAME]\n", 7, 1, fp);
		fwrite("name=", 5, 1, fp);
...
int chatbot_do_forget(int inc, char* inv[], char* response, int n) {
	...
	else									//entered phrase does not exist in linked lists. 
